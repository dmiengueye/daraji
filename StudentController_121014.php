 ...
 */
	...
	public $uses = array ('Student', 'StudentPreference', 'StudentProfile', 'ZipSearch');
                  ...
                  ) 
            {
               ...
               // throw new NotFoundException(__('Zip Code Not Found'));                      
                  ...
                  $this->Paginator->settings = array(
                    ...
                    if (  // similar to findAll(), but fetches paged results
                          (!$data = $this->Paginator->paginate('ZipSearch')) || 
                          (count($data) == 0)
                       ) 
                    {
                        $this->Session->setFlash
                    ...
                    $conditions = array(
                       'Tutor.zip_code' => array(),
                       'Tutor.id' => $this->Auth->user('id')
                       );
                       
                  $this->Paginator->settings = array(
                    'limit' => 10,
                    'conditions' => $conditions,
                    'order' => array('Tutor.zip_code' => 'asc' ));
                       
                     
                       if (  // similar to findAll(), but fetches paged results
                            (!$data = $this->Paginator->paginate('Tutor')) ||  
                            ($data['Tutor']['id'] != $id) ||
                            (count($data) == 0)
                         ) 
                     {
