...
//$this->cacheAction = true;
   		$this->Auth->allow(
						 ...
						 'user_sign_up', 'printSignUrl', 'contact_success', 'message_us', 'message_success'
   			           );
	  ...
	  // debug($this->params->query); 
	   //Check to see if $this->params->query['zip_code'] is in array of Zip_codes
	   //if it is found, do nothing.. if not found, drop it, delete session zip code, compute new zip based on IP address.
	   //Put new zip code in session and replace param zip with new zip code
	   //write code here to drop the cur_zip_code from session if it's not found in zip code table
...
if(empty($cur_zip_code) || $cur_zip_code === "") { // || !$is_valid_zip){
	     ...
	     // debug($is_valid_zip);
	   	...
	   	$ipinfo_api_key = Configure::read('ipinfo_api_key');
	   	$ip_address = '24.125.35.10' ; //$_SERVER['REMOTE_ADDR'];
		
		//$ip_address = $_SERVER['REMOTE_ADDR']; //$this->getUserIP(); 
