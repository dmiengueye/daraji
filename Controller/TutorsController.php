...
public function beforeFilter() {
	...
	//	$this->Security->blackHoleCallback = 'blackhole';
       ...
       // $this->Security->unlockedActions = array('update_entry');
	      ...
	      $this->Session->delete('student_connections');
         //if(!$this->Session->check('student_connections')) {
		  $this->Session->write('student_connections', $this->{$this->modelClass}->get_student_connections($id));
	    //}

      debug($this->Session->read('student_connections'));
      //die();
...
public function submit_lesson() {
 ...
 public function schedule_lesson() {
      ...
      // debug($data); die();
        ...
        $loc =     $data[0]['TutorLocation'];
        $profile = $data[0]['TutorProfile'];
        $connections = $data[0]['StudentsTutor'];
        ...
        **/
         ...
         if(!empty($loc)){
		   ...
		   if(!empty($connections)){
			     $tutor_connections[] = array(0 => 'Pick A Student');
			foreach ($connections as $key => $value) {
				if(!empty($value['student_name']) &&
                  !empty($value['student_id'])){
                    $tutor_connections[] = array($value['student_id'] => $value['student_name']);

				}
			}
		  }
           // $tutor_connections[] = array(0 => 'Pick A Student') + $tutor_connections;
          $tutor_subjects = $this->flatten($tutor_subjects, '');
          $tutor_locations = $this->flatten($tutor_locations, '');
          $tutor_connections = $this->flatten($tutor_connections, '');

         //  debug($tutor_connections); die();
          // debug($tutor_locations);
          // debug($hourly_rate); die();

           asort($tutor_subjects);
           asort($tutor_locations);
		   asort($tutor_connections);

          Configure::write('tutor_locations', $tutor_locations);
          Configure::write('tutor_subjects', $tutor_subjects);
		  Configure::write('tutor_connections', $tutor_connections);
      ...
      // }
        ...
        //if (!empty($this->params->query) && !empty($this->params->query['distance'])) { // if submit
         ...
         //$this->Session->write('$session_search_criteria', $this->params->query);
		  $this->Session->delete("jsauth");
		  $this->Session->delete("jsauthajax");
          $tutor_approved_subjects = $tutors_model->get_all_subjects_for_tutor($this->Auth->user('id'));
		  //debug($tutor_approved_subjects); die();
		  asort($tutor_approved_subjects);
          $uniqueKeys = array();
          foreach($tutor_approved_subjects as $tutorSubject) {
          ...
          $numerical = array();
		  foreach($uniqueKeys as $category => $subjects) {
			 foreach($subjects as $key => $value) {
				$numerical[] = $value;
		     }
		  }
          $uniqueKeys =  $numerical;
          $this->set('tutor_approved_subjects', $uniqueKeys);
		         //debug($uniqueKeys); die();
          //$tutor_approved_subjects = $this->flatten($tutor_approved_subjects, '');
          //debug($tutor_approved_subjects); die();
      ...
      // debug("teststts")    ; die();
       ...
       // debug($this->params->query['subject_id']); die();
        ...
        if( $this->params->query['subject_id'] === self::SUBJECT_ID_100) {
			 ...
			 $this->Session->delete("jsauth"); //not sure if it's lingering in session
			 $this->Session->write('jsauthajax', 'job_search_ajax');
			 $subjects_array = $tutors_model->get_all_subjects_for_tutor($this->Auth->user('id'));
			 $this->Session->delete('jsauthajax');
          ...
          //  debug("here"); die();
             ...
             Configure::write('popularsubjects',$subjects_array);
                 ...
                 if($this->params->query['category_id'] === "MySubjectsGroup") {
					   ...
					   $this->Session->delete("jsauth");
					   $this->Session->delete("jsauthajax");
					   $this->Session->write('jsauthajax', 'job_search_auth_ajax');
                     $subjects_array = $tutors_model->get_all_subjects_for_tutor($this->Auth->user('id'));
					 $this->Session->delete('jsauthajax') ;
                     $return_array = array();
                     foreach ($subjects_array as $key1 => $value1) {
...
protected function _get_nav_data($result_array, $cur_page){
    ...
    $posts_per_page = 2 ; //just change this to control how many items are displayed per page
    $return_value = array();
...
function ajax_subjects() {
   ...
   if (!$this->request->is('ajax')) {
	...
	$this->Session->delete('jsauth');
	//$this->Session->write('jsauth', 'job_search_auth');
    if($id == "MySubjectsGroup") {
		$this->Session->write('jsauthajax', 'mysubjectgroup');
...
public function isAuthorized($user=null) {
	...
	function calendar() {
     //echo "<pre>";
      //debug($this->Session->read('students_connections'));
      //die();
