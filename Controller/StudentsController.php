...
//debug($this->request->data); die();
 if($this->request->is('ajax')) {
  ...
  $category_model = new Categorie();
   ...
   //Registration Data
     $postData[$this->modelClass]['first_name']  = $this->request->data['value']['user']['firstName'];
     $postData[$this->modelClass]['last_name']  =  $this->request->data['value']['user']['lastName'];
   $postData[$this->modelClass]['email'] =       $this->request->data['value']['user']['emailAddress'];
   $postData[$this->modelClass]['zip_code'] =    $this->request->data['value']['zip_subject']['zip_code'];
     $postData[$this->modelClass]['referal'] = '0';
     $postData[$this->modelClass]['referral_source'] = '' ;
     $postData[$this->modelClass]['contactbox'] = '' ;

   // Job Post Data
   $jobPost['StudentJobPost']['student_name'] =    $this->request->data['value']['user']['firstName'].' '.substr($this->request->data['value']['user']['lastName'], 0, 1);
   $jobPost['StudentJobPost']['job_zip_code'] =  $user_zip_code =  $this->request->data['value']['zip_subject']['zip_code'];
   $jobPost['StudentJobPost']['job_description'] = $this->request->data['value']['user']['message'];

   $jobPost['StudentJobPost']['job_subject']     =    $job_subject = $this->request->data['value']['zip_subject']['user_subject'];
     $jobPost['StudentJobPost']['job_subject_id']  =    $subject_model->get_subject_id($job_subject);

   $jobPost['StudentJobPost']['job_category']    =    $this->Session->read('cat_name'); //$job_category = $subject_model->get_category_by_name($job_subject);
   $jobPost['StudentJobPost']['job_category_id'] =    $this->Session->read('cat_id'); //$category_model->get_category_id($job_category);

   $jobPost['StudentJobPost']['job_title'] = 'Need Help with '. $job_subject;

   if(!empty($this->request->data['value']['school'])) {
     $jobPost['StudentJobPost']['school_name'] =  $this->request->data['value']['school'];
   }
   if(!empty($this->request->data['value']['lessonStart'])) {
     $jobPost['StudentJobPost']['lesson_start'] = $this->request->data['value']['lessonStart'];
   }
   if(!empty( $this->request->data['value']['gradeLevel'])) {
     $jobPost['StudentJobPost']['grade_level'] =  $this->request->data['value']['gradeLevel'];
   }
   if(!empty($this->request->data['value']['location'])) {
      $jobPost['StudentJobPost']['lesson_venue'] = $this->request->data['value']['location'];
   }
   if(!empty($this->request->data['value']['availability'])) {
     $jobPost['StudentJobPost']['availability'] =  $this->request->data['value']['availability'];
   }
      ...
      if(empty($this->request->data[$this->modelClass]['request_sign_up'])) {
       ...
       $student_request = null;
        ...
        if (!empty($this->request->data)) {
             Configure::write('Users.role', 'student');
             $this->request->data['Student']['referral_source'] = !empty($this->request->data['Student']['contactbox'])? $this->request->data['Student']['contactbox']: "N/A" ;
...
public function tutor_request($member_id=null) {
   ...
   if($this->request->is('get')) {
     ...
     if(empty($member_id) || (!$tutor = $tutor_model->find('first', array(
       ...
       if(!empty($tutor['TutorImage'][0])) {
        $this->set('tutor_image', $tutor['TutorImage'][0]['image']);
        $this->set('tutor_thumb_image', $tutor['TutorImage'][0]['thumb_image']);
        $this->set('tutor_medium_image', $tutor['TutorImage'][0]['thumb_medium']);
       }
...
public function tutor_search_results() {
  ...
  /**
    if(count($this->request->data)) {
      ...
      if(!$this->request->is('ajax')) {
        header('Location: /users/search/tutor_search_results');
        exit;
      }
    }
  **/
  //debug($this->Session->read('postData')); die();
  
    if($this->Session->read('postData')) {
      $this->request->data = $this->Session->read('postData');
    }
...
// die();
   //if($this->request->is('post')) {
     ...
     if(count($this->request->data)) {
        if(isset($this->request->data['ZipSearch']) && count($this->request->data['ZipSearch'])) {
          $this->params->query = $this->request->data['ZipSearch'];
        } else {
        	if(isset($this->request->data['availability']) && $this->request->data['availability'] != '') {
        		$this->Session->write('availability', $this->request->data['availability']);
        	} else {
        		$this->Session->write('availability', '');
        	}
          $this->params->query = $this->request->data;

        }
     }

     $this->params->query['availability'] = $this->Session->read('availability');

   //This is needed for when the user is coming from SPA
	//The SPA is sending these as shown below but they are stored in DB as a two letter word (ie, High School for HS)
     if(!empty($this->params->query['level']) && $this->params->query['level'] === 'High School')  {
          $this->params->query['level'] = 'HS';
      } else if(!empty($this->params->query['level']) && $this->params->query['level'] === 'Middle School')  {
          $this->params->query['level'] = 'MS';
      } else if(!empty($this->params->query['level']) && $this->params->query['level'] === 'Elem School')  {
         $this->params->query['level'] = 'ES';
      } else if(!empty($this->params->query['level']) && $this->params->query['level'] === 'College')  {
         $this->params->query['level'] = 'CO';
      } else if(!empty($this->params->query['level']) && $this->params->query['level'] === 'Adult')  {
         $this->params->query['level'] = 'Adult';
      }

	  //This is needed for when the user is coming from SPA and chose anything other than "Online"
	  //If anything other than Online/online, it must be an In-Person request therefore an offline meeting

	  if(!empty($this->params->query['location']) &&
	    strcasecmp($this->params->query['location'], "Online") != 0) {
	       $this->params->query['location'] = 'offline';
	  }
	 /** if(!empty($this->params->query['location']) &&
	       ($this->params->query['location'] !== 'Offline' && $this->params->query['location'] !== 'offline') ) {
     ...
     //die;
	 if(!empty($this->params->query['availability']) || !empty($this->params->query['location'])) {
	    $this->params->query['distance'] = 40;
	 }
//      debug($this->params->query); //die();
      if(isset($this->params->query['user_subject']) && !empty($this->params->query['user_subject'])) {
          if(strtolower($this->params->query['user_subject']) === strtolower('Mathematics')) {
            $this->params->query['user_subject'] = 'Math';
          } else if(strtolower($this->params->query['user_subject']) === strtolower('Tech')) {
            $this->params->query['user_subject'] = 'Technology';
          } else {
            $this->params->query['user_subject'] = $this->params->query['user_subject'];
          }
      }
    ...
    if(!empty($this->params->query['user_subject'] ) && $this->params->query['user_subject'] != 'All Subjects') {
      ...
      if(empty( $this->params->query['user_subject_id'] ) )  {
        ...
        $this->params->query['user_subject_id'] = "";
      }
      ...
      if (!empty($this->params->query) && !empty($this->params->query['distance'])) { // if submit with In-Person tutor request
         // debug($this->params->query); die();
        ...
        $this->params->query['user_subject_id'] = $this->params->query['subject_id'];
         ...
         $this->params->query['level'] = !empty($this->params->query['level']) ? $this->params->query['level'] : "all";
         $this->params->query['location'] = !empty($this->params->query['location']) ? $this->params->query['location'] : "all";
    ...
    //debug("dshj"); die();
       if($this->params->query['is_advanced'] == 1){
           $this->params->query['is_advanced'] = !empty($this->params->query['is_advanced']) ? $this->params->query['is_advanced'] : 0;
            ...
            $this->params->query['level'] = !empty($this->params->query['level']) ? $this->params->query['level'] : "all";
            $this->params->query['location'] = !empty($this->params->query['location']) ? strtolower($this->params->query['location']) : "all";

    //debug($this->params->query['location']); die();
     ...
     //debug($this->params->query['gender']); die();
         $this->set('subject', ($this->params->query['subject'])?$this->params->query['subject']:'');
        $this->set('is_advanced', ($this->params->query['is_advanced'])?$this->params->query['is_advanced']:0);
        $this->set('bg_checked', ($this->params->query['bg_checked'])?$this->params->query['bg_checked']:0);
        $this->set('amount_min_rate', ($this->params->query['amount_min_rate'])?$this->params->query['amount_min_rate']:10);
        $this->set('amount_max_rate', ($this->params->query['amount_max_rate'])?$this->params->query['amount_max_rate']:500);
        $this->set('min_age', ($this->params->query['min_age'])?$this->params->query['min_age']:18);
        $this->set('max_age', ($this->params->query['max_age'])?$this->params->query['max_age']:100);
    $this->set('distance', $this->params->query['distance']);
    $this->set('gender', $this->params->query['gender']);
        $this->set('location', ($this->params->query['location'])? strtolower($this->params->query['location']):'offline');
    $this->set('level', ($this->params->query['level'])?$this->params->query['level']:0);
    $this->set('availability', (isset($this->params->query['availability']))?$this->params->query['availability']:'');
...
//    debug($this->params->query); die();
  ...
  **/
  ...
  //$this->params->query['user_subject'] =
    ...
    //debug( $return_array); die();user_subject
	   $this->set('tutor_size',  $result);
      $this->set('tutors', $return_array);
      $this->set('subject', ($this->params->query['subject'])?$this->params->query['subject']:'');
      $this->set('zip', $this->Session->read('cur_zip_code'));
...
public function tutor_details_profile($member_id=null) {
    ...
    //debug($tutor); die();
       ...
       if(!empty($tutor) && !empty($tutor['TutorProfile']) && !empty($tutor['Tutor'])) {
              ...
              $this->set('degree', $tutor['TutorProfile']['degree']);
              $this->set('school', $tutor['TutorProfile']['school']);
              $this->set('education', $tutor['TutorProfile']['education']);
              $this->set('hourly_rate', $tutor['TutorProfile']['hourly_rate']);
              $this->set('travel_radius', $tutor['TutorProfile']['travel_radius']);
              $this->set('bg_checked', $tutor['TutorProfile']['background_checked']);
              $this->set('rating_score', $tutor['TutorProfile']['avg_rating_score']);
	      ...
	      if( !empty($tutor['TutorProfile']['lesson_location']) && $tutor['TutorProfile']['lesson_location'] === strtolower("Offline")) {
			  //if( !empty($tutor['TutorProfile']['lesson_location']) && strcasecmp($tutor['TutorProfile']['lesson_location'] , "Offline") ==0) {
				  $tutor['TutorProfile']['lesson_location'] = "In-Person";
	      } else if( !empty($tutor['TutorProfile']['lesson_location']) && $tutor['TutorProfile']['lesson_location'] === strtoupper("ANY")) {
		  //if( !empty($tutor['TutorProfile']['lesson_location']) && strcasecmp($tutor['TutorProfile']['lesson_location'] , "ANY") ==0) {
				$tutor['TutorProfile']['lesson_location'] = "In-Person and Online";
	      } else if( !empty($tutor['TutorProfile']['lesson_location']) && $tutor['TutorProfile']['lesson_location'] === strtolower("Online")) {
		     //if( !empty($tutor['TutorProfile']['lesson_location']) && strcasecmp($tutor['TutorProfile']['lesson_location'] , "Online") ==0) {
				  $tutor['TutorProfile']['lesson_location'] = "Teaches Online Only";
	      }
	     $this->set('online_offline',    h($tutor['TutorProfile']['lesson_location']));
        ...
        // debug($tutor_subjects); die();
                 ...
                 // format
                  //echo "<pre>";
                  //print_r($tutor_subjects);die;


                 //print_r($tutor_subjects);
                 $uniqueKeys = array();
                 foreach($tutor_subjects as $tutorSubject) {
                  $tutorSubject = $tutorSubject[0];
                   foreach($tutorSubject as $key => $value) {
                      //echo 'else '.$key.' '.$value."<br/>";
                      if (array_key_exists($key, $uniqueKeys)) {
                        //echo 'Here in if';
                        array_push($uniqueKeys[$key], $value);
                      } else {

                          $uniqueKeys[$key] = array();
                          array_push($uniqueKeys[$key], $value);
                      }
                   }
               }
                 ...
                 //die;
                  ...
                  //die;
                 $this->set('tutor_subjects', $uniqueKeys);
...
protected function flatten($array, $prefix = '') {
  ...
  **/
  ...
  protected function _get_nav_data($result_array, $cur_page){
    ...
    $posts_per_page = 2 ;  //Just change this number to control how many entries are displayed per page
    $return_value = array();
	     ...
	     if (!empty($this->request->data)) {
                                ...
                                'StudentJobPost.id'  => $id)))) )
                     {
                           ...
                           // throw new NotFoundException(__('Job Description Must be a New One'));
							$this->set('success', false);
                            $this->Session->setFlash('Our records show that this user does not exist', 'custom_msg');
                     }
                    ...
                    if($this->{$this->modelClass}->StudentJobPost->validates(array('fieldList' => array(
                                                            ...
                                                            'job_subject_id' ))))
                      {
						...
						if($this->Session->check('username')) {
							 ...
							 $this->request->data['StudentJobPost']['student_name'] = $first_name.' '.$last_name;
                        }

						if($this->Session->check('student_state')) {
							$this->request->data['StudentJobPost']['job_state'] = $job_state =  $this->Session->read('student_state') ;
                            $job_id  = $job_state.$result[1];
						} else if (!empty($this->request->data['StudentJobPost']['job_state'])){
							$job_id  = $this->request->data['StudentJobPost']['job_state'].$result[1];
						}else {
							$job_id  = $result[1];
						}
						if($this->Session->check('student_city')) {
							$this->request->data['StudentJobPost']['job_city'] = $this->Session->read('student_city') ;
						}
						if($this->Session->check('job_zip_code')) {
							$this->request->data['StudentJobPost']['job_zip_code'] = $this->Session->read('job_zip_code') ;
						}
					  ...
					  if($this->{$this->modelClass}->StudentJobPost->saveJobPost($id, $this->request->data))
					   {
							if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) &&  strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
								...
								exit;
								  ...
								  $this->set('success', true);
                                  $this->Session->setFlash('Your job was successfully submitted and pending review. You will be notified of its Status shortly.', 'custom_msg');
							      $this->redirect($this->referer('/'));
							 }
					  }
                  } else {
                            if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) &&  strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
							  ...
							  $_SESSION['errors'] = $this->{$this->modelClass}->StudentJobPost->validationErrors;
							  if(count($_SESSION['errors']) > 0) {
							   ...
							   // debug($_SESSION['errors'] = $this->{$this->modelClass}->validationErrors)	; die();
							  $this->Session->write('error_array', $this->{$this->modelClass}->StudentJobPost->validationErrors);
                              $this->set('success', false);
...
function save_data() {
   ...
   if (!$this->request->is('ajax')) {
    ...
    //debug($this->request->data['value']); die();
 /**$this->{$this->modelClass}->set(array(
                                           'first_name' =>  $this->request->data['value']['user']['firstName'],
                                           'last_name' =>     $this->request->data['value']['user']['lastName'],
                                           'email' =>      $this->request->data['value']['user']['email'],
                                           'zip_code' =>        $this->request->data['value']['zip_subject']['zip_code']

                                  ));



         if($this->{$this->modelClass}->validates(array('fieldList' => array(
                                                                     'first_name',
                                                             'last_name',
                                                                     'email',
                                                                     'zip_code'))))
         {
                 ...
                 $this->{$this->modelClass}->saveField('first_name', $this->request->data['value']['user']['firstName']);
                 $this->{$this->modelClass}->saveField('last_name', this->request->data['value']['user']['lastName']);
                 $this->{$this->modelClass}->saveField('email', $this->request->data['value']['user']['email']);
                 $this->{$this->modelClass}->saveField('zip_code', $this->request->data['value']['zip_subject']['zip_code']);

         } else {
                    $this->Session->write('validation_error_array', $this->{$this->modelClass}->validationErrors);
                    $this->redirect($this->referer('/'));
                }
...
public function zip_subject() {
  ...
  public function ajax_zip_subjects() {
    //echo "Here";
    //die;
  //debug("hsdh"); die();

    $this->layout = 'ajax_spa';
    $this->view = 'zip_subject';
  }

  public function ajax_grade_level() {
    $this->layout = 'ajax_spa';
    $this->view = 'grade_level';
  }

  public function ajax_lesson_start() {
    $this->layout = 'ajax_spa';
    $this->view = 'lesson_start';
  }
  ...
  public function ajax_lesson_location() {
      $this->layout = 'ajax_spa';
      $this->view = 'lesson_location';
   }

   public function ajax_student_school() {
      $this->layout = 'ajax_spa';
      $this->view = 'student_school';
   }

   public function ajax_final_reg_form() {
      $this->layout = 'ajax_spa';
      $this->view = 'final_reg_form';
   }

   public function ajax_introduction() {
    $this->layout = 'ajax_spa';
      $this->view = 'ajax_introduction';
   }
  ...
  public function lesson_location() {
      $this->layout = 'spa';
   }
   ...
   public function student_school() {
     ...
     public function final_reg_form() {
      $this->layout = 'spa';
   }
