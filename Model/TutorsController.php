          ...
          if(!empty($tProfileModel)) {
           ...
           if(empty($pProfileStatus) || !$pProfileStatus ) {
            ...
            if(!empty($tProfileModel)) {
                       ...
                       $this->Session->write('ica_status',    h($tProfileModel['TutorProfile']['ica_status']));
                       $this->Session->write('profile_status_count',    h($tProfileModel['TutorProfile']['profile_status_count']));
       ...
       $tProfileModel = $this->{$this->modelClass}->TutorProfile->find('first', array(
   	      ...
   	      if(!empty($tProfileModel)) {
                    ...
                    //debug($tProfileModel['TutorProfile']['profile_status_count']); die();
                     // $this->render('/elements/tutor_profile_steps');
        ...
        //debug($this->request->data); die();
          ...
          //I had to change this on front end through the use $job_post_date_hidden
           ...
           $job_post_date = new DateTime($job_post_date);
            if(!$job_post_date) {
                throw new NotFoundException(__('Invalid Post Date'));
            } else {
          ...
          $this->request->data['TutorJobApplication']['profile_link'] = $full_url = Router::url(array('controller' => 'tutors', 'action'=>'tutor_details_profile_auth/', 3), true);
          ...
          $this->{$this->modelClass}->TutorJobApplication->set(array(
                                   ...
                                   'applicant_zip_code' => $this->request->data['TutorJobApplication']['profile_zip_code'],
...
function send_emails($userData, $options=array()) {
    //debug("In"); die();
    
    	$defaults = array(
			'from' => Configure::read('App.defaultEmail'),
			'subject' => __d('students', $userData['TutorJobApplication']['applicant_name'].' has applied for your '.$userData['TutorJobApplication']['job_subject'].' request on Daraji'),
			'template' => $this->_pluginDot() . 'job_application',
			'layout' => 'default',
 	...
 	$options = array_merge($defaults, $options);
		...
		$Email = $this->_getMailInstance();
         //debug($userData); die();
		$Email->to($userData['TutorJobApplication']['student_email']);
   ...
   if($this->request->is('get')) {
    ...
    //query the tutor_job_application table and pull the job app details (App_date and subject_name)
      ...
      if(!empty($job_app_details)) {
        $this->set('job_id',          $job_app_details[0]['job_id']);
        $this->set('job_category',     $job_app_details[0]['job_category']);
        $this->set('subject_name',     $job_app_details[0]['job_subject']);
        $this->set('job_desc',         $job_app_details[0]['job_desc']);

        $this->set('job_title',        $job_app_details[0]['job_title']);
        $this->set('job_post_date',    $job_app_details[0]['job_post_date']);
        $this->set('job_requester',    $job_app_details[0]['job_requester']);

        $this->set('job_city',         $job_app_details[0]['job_city']);
        $this->set('job_state',        $job_app_details[0]['job_state']);
        $this->set('job_zip_code',     $job_app_details[0]['job_zip_code']);
        $this->set('job_applicant',    $job_app_details[0]['job_applicant']);

        $this->set('personal_message',     $job_app_details[0]['personal_message']);
        $this->set('subject_creds',        $job_app_details[0]['subject_creds']);
        $this->set('job_application_date', $job_app_details[0]['application_date']);

        $this->set('applicant_ratings',          $job_app_details[0]['applicant_ratings']);
        $this->set('applicant_avg_rating_score', $job_app_details[0]['applicant_avg_rating_score']);
...
public function job_details_auth($job_id=null) {
    ...
    if($this->request->is('get')) {
       ...
       if(empty($tutor_profile) || $tutor_profile['TutorProfile']['profile_status_count'] != '4' )  {
        ...
        $job_details = $students_model->find_job_by_id($job_id);
        ...
        if(!empty($tutor_profile)) {
         ...
         // $zip_code = $this->Session->read('zip_code');
          ...
          $this->set('city', $tutor_profile['city']);
          $this->set('state', $tutor_profile['state']);
          $this->set('profile_zip_code',  $tutor_profile['zip_code']); 
          $this->set('degree', $tutor_profile['degree']);
          $this->set('school', $tutor_profile['school']);
          $this->set('hourly_rate', $tutor_profile['hourly_rate']);
          $this->set('travel_radius', $tutor_profile['travel_radius']);
          $this->set('bg_checked', $tutor_profile['background_checked']);
          $this->set('rating_score', $tutor_profile['avg_rating_score']);
          $this->set('ratings', $tutor_profile['ratings']);
         } 
...
/*
	...
	* @return      :null
	*/
function job_search_agent($id=null, $agent_id=null){
 ...
 if($this->request->is('post')) {
    ...
    // debug($id); die();
     ...
     $this->Session->write('id', $id);
      ...
      return $this->redirect(array('action' => 'job_search_agent'));
...
public function delete_agent($id=null) {
 ...
 protected function getAllJobSearchAgents($id) {
  ...
  protected function my_job_agents() {
    
    $this->layout='tutor';
    if($this->request->is('get')) {
        
         $job_search_agents = null; //$search_agent_model;
         $job_search_agents  = $this->findJobSearchAgents($this->Auth->user('id'));
         $this->set('job_search_agents', $job_search_agents);
     }
