...
public function findSubjectCategory($subject_id, $subject_name){
//debug($subject_id);
//debug($subject_name);
  $data = $this->find('all',
            array(
...
//debug($data); die();
  if(!empty($data)){
    if(!empty($data[0]['Subject']['subject_id'])){
        CakeSession::write('cat_id', $data[0]['Subject']['category_id']);
        CakeSession::write('cat_name', $data[0]['Subject']['category_name']);
      return $data[0]['Subject']['subject_id'];
    }
    else{
        return 0;
    }
